var aboutButton = document.getElementById("aboutme-button");
var homeButton = document.getElementById("home-button");
var projectButton = document.getElementById("projects-button");
...
var fadeOutSpeed = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--fade-speed').slice(0, -2));
...
function LoadElements(state)
{
	if (state === StateEnum.Home) {
		...
		fetch("sections/aboutme.html").then(results =>
		{
			return results.text();
		}).then(str =>
		{
...
function Reset()
{
	...
	{
		homeButton.classList.remove("selected-menu-item");
		var centerDiv = document.getElementById("home-section");
		...
		var aboutSection = document.getElementById("aboutme-section");
...
/*function updateSelectedTextHeight() {
	//console.log("BASE HEIGHT = " + baseTitleHeight);
	var currentHeight = selectedProjectTitle.clientHeight;
	var changeFactor = 1.0 / Math.round(currentHeight / baseTitleHeight);
	console.log("CURRENT HEIGHT = " + currentHeight);
	//console.log("Change Factor = " + changeFactor);
	//console.log("OVERFLOW FACTOR = " + Math.round(currentHeight / baseTitleHeight));

	currentFontSize *= changeFactor;
	//baseTitleHeight *= currentHeight;

	root.style.setProperty('--selected-project-title-size', currentFontSize + "cm");
}

window.onresize = updateSelectedTextHeight;*/
